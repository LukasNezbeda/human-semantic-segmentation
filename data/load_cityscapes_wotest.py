...
OUTPUT_SPLIT_MAP = {"train": "train", "val": "valid"}
...
def split_validation_pairs(
    ...
    """Split validation pairs into valid/test halves."""
    rng = random.Random(seed)
    shuffled = list(pairs)
    rng.shuffle(shuffled)
    half = len(shuffled) // 2
    test_pairs = shuffled[:half]
    valid_pairs = shuffled[half:]
...
def main(argv: Sequence[str] | None = None) -> int:
	...
	for split in INPUT_SPLITS:
		...
		if split == "train":
			write_split(pairs, args.output_root, "train", args.height, args.width)
			split_pairs["train"] = pairs
		else:
			valid_pairs, test_pairs = split_validation_pairs(pairs, seed=RANDOM_STATE)
			write_split(valid_pairs, args.output_root, "valid", args.height, args.width)
			write_split(test_pairs, args.output_root, "test", args.height, args.width)
			split_pairs["valid"] = valid_pairs
